-- Harden checkout for Moroccan cash handling.
...
drop function if exists public.pos_checkout(numeric,numeric,numeric,text,jsonb,uuid);
...
begin
  ...
  if p_amount_received is null or p_amount_received < 0 then raise exception 'Invalid amount received'; end if;
  ...
  if p_shift_id is not null then
    select user_id into v_shift_user from public.cash_register_shifts where id=p_shift_id and status='open';
    if v_shift_user is distinct from auth.uid() then raise exception 'Open cash shift required'; end if;
  end if;
  ...
  if round(p_total,2) <> v_total then raise exception 'Invalid total'; end if;
  if p_payment_method='cash' then
    if p_amount_received < v_total then raise exception 'Cash received is less than total'; end if;
  ...
  values(auth.uid(),v_subtotal,p_discount,v_total,v_profit,p_payment_method,p_customer_id,p_shift_id,p_amount_received,v_change)
  ...
  values(auth.uid(),'create','sale',v_sale_id,jsonb_build_object('total',v_total,'payment_method',p_payment_method,'amount_received',p_amount_received,'change',v_change));
  return v_sale_id;
end; $$;
