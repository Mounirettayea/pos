  ...
  @override Widget build(BuildContext context) => Scaffold(
        body: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: ConstrainedBox(
          constraints: const BoxConstraints(maxWidth: 430), child: Column(children: [
            const Icon(Icons.spa, size: 72), const SizedBox(height: 10),
            const Text('MAISON AL TEEB POS', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)),
            const Text('POS • Morocco'), const SizedBox(height: 28),
            ...
            const SizedBox(height: 18), SizedBox(width: double.infinity, child: FilledButton(onPressed: loading ? null : submit, child: Text(loading ? '...' : 'دخول'))),
          ]),
        )),
      );
}
...
class _RoleGateState extends State<RoleGate> {
  ...
  Future<void> _load() async {
    ...
    } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Role error: $e'))); await db.auth.signOut(); }
...
class _ShiftGateState extends State<ShiftGate> {
  ...
  Future<void> _load() async {
    try { shift = await db.from('cash_register_shifts').select().eq('user_id', db.auth.currentUser!.id).eq('status', 'open').maybeSingle(); }
    catch (e) { _msg('تعذر تحميل الصندوق: $e'); }
    if (mounted) setState(() => loading = false);
  }
  Future<void> _open() async {
    final amount = double.tryParse(opening.text.replaceAll(',', '.'));
    if (amount == null || amount < 0) return _msg('دخل مبلغ صحيح');
    try {
  ...
  @override Widget build(BuildContext context) {
    ...
    if (shift == null) return Scaffold(body: Center(child: Card(margin: const EdgeInsets.all(24), child: Padding(padding: const EdgeInsets.all(24), child: ConstrainedBox(
      constraints: const BoxConstraints(maxWidth: 420), child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.point_of_sale, size: 64), SizedBox(height: 12), Text('فتح صندوق البداية', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)), SizedBox(height: 8),
        Text('دخل المبلغ الموجود في الصندوق قبل بداية الخدمة.'), SizedBox(height: 18),
        TextField(controller: opening, keyboardType: TextInputType.numberWithOptions(decimal: true), decoration: InputDecoration(labelText: 'Opening cash (MAD)')),
...
class _HomePageState extends State<HomePage> {
  ...
  String search = '';
  int tab = 0;
  double get total => cart.fold<double>(0, (s, x) => s + ((x.product['sell_price'] as num?)?.toDouble() ?? 0) * x.qty);
  ...
  List<Map<String, dynamic>> get filtered {
    ...
    return products.where((p) => '${p['name'] ?? ''} ${p['name_ar'] ?? ''} ${p['name_fr'] ?? ''} ${p['barcode'] ?? ''} ${p['sku'] ?? ''}'.toLowerCase().contains(q)).toList();
  }
  void addProduct(Map<String, dynamic> p) {
    final stock = (p['stock'] as num?)?.toInt() ?? 0;
    if (stock <= 0) return _msg('المنتج سالا من الستوك');
  ...
  Future<void> scan() async {
    ...
    try {
      final p = await db.from('products').select('id,name,name_ar,name_fr,barcode,sku,sell_price,buy_price,stock,min_stock,category,image_url').eq('barcode', code).maybeSingle();
      if (p == null) _msg('Barcode غير موجود'); else addProduct(Map<String, dynamic>.from(p));
    } catch (e) { _msg('خطأ فالبحث: $e'); }
  }
  Future<void> checkout() async {
    if (cart.isEmpty) return;
    final result = await showDialog<Map<String, dynamic>>(context: context, builder: (_) => PaymentDialog(total: total));
    ...
    try {
      ...
      setState(() => cart.clear());
      await loadProducts();
      if (mounted) _msg('تم البيع بنجاح • ${amount.toStringAsFixed(2)} MAD');
    } catch (e) { _msg('تعذر إتمام البيع: $e'); }
  }
  Future<void> closeShift() async {
    final c = TextEditingController();
    final actual = await showDialog<double>(context: context, builder: (_) => AlertDialog(title: const Text('إغلاق الصندوق'), content: TextField(controller: c, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Cash الموجود فعلياً (MAD)')), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('إلغاء')), FilledButton(onPressed: () => Navigator.pop(context, double.tryParse(c.text.replaceAll(',', '.'))), child: const Text('إغلاق'))]));
    c.dispose();
    if (actual == null) return;
    try {
      final s = await db.from('cash_register_shifts').select('opening_cash').eq('id', widget.shiftId).single();
      final rows = await db.from('sales').select('total').eq('shift_id', widget.shiftId).eq('payment_method', 'cash');
  ...
  void _msg(String s) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(s))); }
  @override Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('MAISON AL TEEB POS'), actions: [Center(child: Text(widget.role.toUpperCase())), IconButton(onPressed: loadProducts, icon: const Icon(Icons.refresh)), IconButton(onPressed: closeShift, icon: const Icon(Icons.lock_clock)), IconButton(onPressed: () => db.auth.signOut(), icon: const Icon(Icons.logout))]),
...
class _PaymentDialogState extends State<PaymentDialog> {
  ...
  @override void initState() { super.initState(); received.text = widget.total.toStringAsFixed(2); }
  @override void dispose() { received.dispose(); super.dispose(); }
  @override Widget build(BuildContext context) => AlertDialog(title: const Text('الدفع'), content: Column(mainAxisSize: MainAxisSize.min, children: [Text('Total: ${widget.total.toStringAsFixed(2)} MAD'), const SizedBox(height: 12), DropdownButtonFormField<String>(value: method, items: const [DropdownMenuItem(value: 'cash', child: Text('Cash')), DropdownMenuItem(value: 'card', child: Text('Card')), DropdownMenuItem(value: 'transfer', child: Text('Transfer'))], onChanged: (v) => setState(() => method = v ?? 'cash')), const SizedBox(height: 12), TextField(controller: received, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Amount received (MAD)'))]), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('إلغاء')), FilledButton(onPressed: () { final v = double.tryParse(received.text.replaceAll(',', '.')); if (v == null || v < widget.total) return; Navigator.pop(context, {'method': method, 'received': v}); }, child: const Text('تأكيد'))]);
}

class ScannerPage extends StatelessWidget {
  const ScannerPage({super.key});
  @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Scan barcode')), body: MobileScanner(onDetect: (capture) { if (capture.barcodes.isEmpty) return; final code = capture.barcodes.first.rawValue; if (code != null && code.isNotEmpty) Navigator.pop(context, code); }));
}
