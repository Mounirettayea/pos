  ...
  @override Widget build(BuildContext context) => Scaffold(
    body: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: ConstrainedBox(
      constraints: const BoxConstraints(maxWidth: 430), child: Column(children: [
        Icon(Icons.spa, size: 72), const SizedBox(height: 10),
...
class _HomePageState extends State<HomePage> {
  ...
  List<Map<String, dynamic>> get filtered => products.where((p) { final q = search.trim().toLowerCase(); if (q.isEmpty) return true; return '${p['name'] ?? ''} ${p['name_ar'] ?? ''} ${p['name_fr'] ?? ''} ${p['barcode'] ?? ''} ${p['sku'] ?? ''}'.toLowerCase().contains(q); }).toList();
