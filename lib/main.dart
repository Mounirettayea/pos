    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Role error: $e')),
        );
      }
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text(
                      'دخل المبلغ الموجود في الصندوق قبل بداية الخدمة.',
                    ),
                    const SizedBox(height: 18),
                    TextField(
                      controller: opening,
...
class _HomePageState extends State<HomePage> {
  ...
  double get total => cart.fold<double>(
        0,
        (sum, line) =>
            sum + ((line.product['sell_price'] as num?)?.toDouble() ?? 0) * line.qty,
      );
  ...
  Future<void> loadProducts() async {
    ...
    try {
      final rows = await db
          .from('products')
          .select(
            'id,name,name_ar,name_fr,barcode,sku,sell_price,buy_price,stock,min_stock,category,image_url',
          )
          .order('name');
      products = List<Map<String, dynamic>>.from(rows);
    } catch (e) {
      _msg('تعذر تحميل المنتجات: $e');
    }
    if (mounted) setState(() => loading = false);
  }
  ...
  List<Map<String, dynamic>> get filtered {
    ...
    return products.where((p) {
      final text = '${p['name'] ?? ''} ${p['name_ar'] ?? ''} '
          '${p['name_fr'] ?? ''} ${p['barcode'] ?? ''} ${p['sku'] ?? ''}'
          .toLowerCase();
    ...
    try {
      ...
      await showDialog<void>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text(
            'المجموع: ${amount.toStringAsFixed(2)} MAD\n'
            'المستلم: ${received.toStringAsFixed(2)} MAD\n'
            'الباقي: ${method == 'cash' ? (received - amount).toStringAsFixed(2) : '0.00'} MAD',
          ),
          actions: [
            TextButton(
  ...
  Future<void> closeShift() async {
    ...
    final actual = await showDialog<double>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
            labelText: 'Cash الموجود فعلياً (MAD)',
          ),
        ),
        actions: [
          TextButton(
    ...
    try {
      final shift = await db
          .from('cash_register_shifts')
          .select('opening_cash,opened_at')
          .eq('id', widget.shiftId)
          .single();
      final opened = DateTime.parse(shift['opened_at'].toString())
          .toUtc()
          .toIso8601String();
      final rows = await db
          .from('sales')
      ...
      final cashSales = rows.fold<double>(
        0,
        (sum, row) => sum + ((row['total'] as num?)?.toDouble() ?? 0),
      );
      final expected = (shift['opening_cash'] as num).toDouble() + cashSales;
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: Center(child: Text(widget.role.toUpperCase())),
          ),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...cart.map((line) {
              final price =
                  (line.product['sell_price'] as num?)?.toDouble() ?? 0;
              return Row(
                children: [
                  Expanded(
                    child: Text(
                  ...
                  Text('${line.qty}'),
                  IconButton(
                    onPressed: () => addProduct(line.product),
                    icon: const Icon(Icons.add_circle_outline),
                  ),
                  const SizedBox(width: 8),
                  Text('${(price * line.qty).toStringAsFixed(2)} MAD'),
                ],
              );
            }),
            const Divider(),
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                label: const Text('الدفع وتأكيد البيع'),
              ),
            ),
          ],
        ),
  ...
  Widget _dashboard() {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: db
          .from('sales')
          .select('id,total,profit,payment_method,created_at,receipt_number')
      ...
      builder: (_, snapshot) {
        ...
        if (snapshot.hasError) {
          return Center(child: Text('Dashboard error: ${snapshot.error}'));
        }
        ...
        final profit = rows.fold<double>(
          0,
          (sum, row) => sum + ((row['profit'] as num?)?.toDouble() ?? 0),
        );
        final low = products.where((product) {
          final stock = (product['stock'] as num?)?.toInt() ?? 0;
        ...
        return ListView(
          ...
          children: [
            _Kpi('المبيعات الأخيرة', '${revenue.toStringAsFixed(2)} MAD', Icons.payments),
            _Kpi('الربح', '${profit.toStringAsFixed(2)} MAD', Icons.trending_up),
            _Kpi('Low stock', '$low', Icons.warning),
            const SizedBox(height: 12),
            const Text(
              'آخر العمليات',
            ...
            ...rows.map(
              (row) => ListTile(
                ...
                title: Text(
                  '${((row['total'] as num?)?.toDouble() ?? 0).toStringAsFixed(2)} MAD',
                ),
                subtitle: Text(
                  'Receipt #${row['receipt_number'] ?? '-'} • '
                  '${row['payment_method'] ?? ''} • ${row['created_at'] ?? ''}',
                ),
              ),
            ),
          ],
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            value: method,
            decoration: const InputDecoration(labelText: 'طريقة الأداء'),
            items: const [
          ...
          TextField(
            ...
            decoration: const InputDecoration(labelText: 'المبلغ المستلم MAD'),
          ),
          if (method == 'cash') ...[
            const SizedBox(height: 10),
            Align(
              ...
              child: Text(
                'Change: ${change >= 0 ? change.toStringAsFixed(2) : '0.00'} MAD',
              ),
            ),
          ],
        ],
      ...
      actions: [
        ...
        FilledButton(
          onPressed: method == 'cash' && amount < widget.total
              ? null
              : () => Navigator.pop(context, {
                    'method': method,
                    'received': method == 'cash' ? amount : widget.total,
                  }),
